...
const MapView = () => {
  ...
  return (
    <div className="rounded-xl border border-border overflow-hidden bg-white shadow-sm h-64 relative">
      <div className="p-4 absolute z-10 top-0 left-0 right-0 bg-gradient-to-b from-background/80 to-transparent">
      ...
      {!mapboxToken ? (
        ...
        <div className="h-full w-full bg-muted flex items-center justify-center">
          <p className="text-sm text-muted-foreground">Map would load here with token: {mapboxToken.slice(0, 5)}...</p>
        </div>
      )}
    </div>
  );
